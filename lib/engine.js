...
export async function generateImage({ prompt, size = "portrait", references = [] }) {
  if (!openaiKey()) throw new Error("OPENAI_API_KEY no configurado en Vercel (necesario para las imágenes).");
  const px = size === "square" ? "1024x1024" : "1024x1536";
  const quality = (process.env.OPENAI_IMAGE_QUALITY || "medium").trim();
  const preferred = (process.env.OPENAI_IMAGE_MODEL || "gpt-image-2").trim();
