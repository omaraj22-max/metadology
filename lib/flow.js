...
- Si es el primer mensaje, preséntate en una línea (Aria, de Caperifai), di que con 4 preguntas le armas un anuncio listo para Meta con la identidad de su marca, y haz la pregunta 1. No menciones la palabra "moodboard".
...
- Cuando tengas los 4 datos: confirma en una línea lo que entendiste (producto + país) y avisa que en unos minutos le mandas por aquí su anuncio. No preguntes nada más.
...
function fallbackTurn(conv, text) {
  ...
  let replyText;
  if (isFirst) {
    replyText = `¡Hola${conv.name ? " " + conv.name.split(" ")[0] : ""}! Soy Aria, la IA de Caperifai. Con 4 preguntas te armo un anuncio listo para Meta con la identidad de tu marca. Primero: ${QUESTIONS.producto}`;
  } else if (next) {
    replyText = QUESTIONS[next];
  } else {
    replyText = `Perfecto: *${fields.producto}* en ${fields.pais}. Dame unos minutos y te mando por aquí tu anuncio.`;
  }
...
export async function handleInbound(msg) {
  ...
  if (conv.status === "generating") {
    await reply(conv, "Sigo trabajando en tu anuncio ✨ En unos minutos te lo mando por aquí.");
    await saveConversation(conv);
    return;
  }
