...
export function ctaUrl() {
  return process.env.CAPERIFAI_CTA_URL || "https://calendar.app.google/oqKtfT6Hkv4eET5u8";
}
export function internalSecret() {
  return (process.env.WA_INTERNAL_SECRET || process.env.META_WEBHOOK_VERIFY_TOKEN || "dev-secret").trim();
}
...
export async function triggerStage(convId, stage) {
  const res = await fetch(`${siteUrl()}/api/wa/process`, {
    method: "POST",
...
export async function handleInbound(msg) {
  ...
  if (conv.status === "done") {
    await reply(
      conv,
      `Tu resultado completo está aquí: ${siteUrl()}/r/${conv.resultId}\n\n¿Quieres que lo llevemos a una campaña completa? Agenda con el equipo de Caperifai: ${ctaUrl()}\n\n(Si quieres analizar otra marca, escribe *reiniciar*.)`
    );
    await saveConversation(conv);
    return;
  }
