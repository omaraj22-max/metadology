...
export function imageUrl(id) {
  return `${siteUrl()}/api/wa/image/${id}`;
}
...
async function stageMoodboard(conv, r) {
  ...
  await saveResult(r);
  try {
    await sendImage(conv.phone, imageUrl(r.moodboardImg), `Tu moodboard: la piel de ${r.brand?.marca || "tu marca"} 🎨\n\nCon esto armo tu anuncio, ya casi…`);
    conv.transcript.push({ role: "aria", text: `[imagen: moodboard] ${imageUrl(r.moodboardImg)}`, at: Date.now() });
  } catch (e) {
...
async function stageAd(conv, r) {
  ...
  const link = `${siteUrl()}/r/${r.id}`;
  try {
    await sendImage(conv.phone, imageUrl(r.adImg), `🎉 Listo${firstName(conv) ? ", " + firstName(conv) : ""}. Tu anuncio Metadology para *${r.brand?.marca || "tu marca"}*.`);
    conv.transcript.push({ role: "aria", text: `[imagen: anuncio] ${imageUrl(r.adImg)}`, at: Date.now() });
  } catch (e) {
    console.error("[wa] imagen anuncio:", e?.message || e);
  }
  await reply(
    conv,
    `*Copy listo para pegar en Meta:*\n\n*Hook:* ${c.hook || ""}\n*Valor:* ${c.valor || ""}\n*Oferta:* ${c.oferta || ""}\n*CTA:* ${c.cta || ""}\n\n${r.campaign.explicacion || ""}\n\nTu resultado completo (moodboard, ángulos de venta, conceptos y estrategia): ${link}\n\nEsto fue 1 anuncio; una campaña ganadora necesita 8-12. ¿Lo armamos juntos? Agenda con Caperifai: ${ctaUrl()}`
  );
  conv.status = "done";
  conv.stage = "done";
}
