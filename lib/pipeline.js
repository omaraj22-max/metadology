...
async function stageMoodboard(conv, r) {
  ...
  try {
    ...
    await sendImage(conv.phone, url, `Aquí está la piel de ${r.brand?.marca || "tu marca"} 🎨 — colores, tipografía y estilo. Con esto armo tu anuncio, ya casi…`);
    conv.transcript.push({ role: "aria", text: `[imagen: piel de marca] ${url}`, at: Date.now() });
  } catch (e) {
...
async function stageCampaign(conv, r) {
  ...
  await reply(
    conv,
    `Ya tengo tu ángulo ganador${lente ? `: *${lente}*` : ""} 🎯${c.hook ? `\n\n«${c.hook}»` : ""}\n\nEstoy renderizando tu anuncio con la piel de tu marca; es lo último ✨`
  );
}
...
async function stageAd(conv, r) {
  ...
  const img = await generateImage({ prompt: r.campaign.imagePrompt, size: "portrait", references: refs, quality: await getSetting("OPENAI_AD_QUALITY") });
  ...
  // Último mensaje: resumen ejecutivo (persona, consciencia, deseo, ángulo ganador, 3 ángulos, lanzamiento).
  await reply(conv, buildExecutiveSummary({ marca: r.brand?.marca, campaign: r.campaign, link }));
  conv.status = "done";
  conv.stage = "done";
}
