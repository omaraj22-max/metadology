...
/** Bitácora: agrega al inicio y recorta a `keep` elementos. */
export async function logPush(key, value, keep = 40) {
  if (hasRedis()) {
    await redis(["LPUSH", key, JSON.stringify(value)]);
    await redis(["LTRIM", key, "0", String(keep - 1)]);
    return;
  }
  const db = await localRead();
  db.list = db.list || {};
  db.list[key] = [value, ...(db.list[key] || [])].slice(0, keep);
  await localWrite(db);
}

export async function logRead(key, keep = 40) {
  if (hasRedis()) {
    const raw = (await redis(["LRANGE", key, "0", String(keep - 1)])) || [];
    return raw.map((r) => { try { return JSON.parse(r); } catch { return null; } }).filter(Boolean);
  }
  const db = await localRead();
  return (db.list?.[key] || []).slice(0, keep);
}

/** Ping al store: sirve para el diagnóstico. */
export async function storePing() {
  const started = Date.now();
  await kvSet("wa:ping", { at: started }, { ex: 60 });
  const back = await kvGet("wa:ping");
  return { ok: back?.at === started, ms: Date.now() - started, backend: hasRedis() ? "redis" : "archivo local" };
}
...
export const K = {
  ...
  hooks: "wa:hooks", // bitácora de llamadas al webhook (diagnóstico)
};
